...
| `numanager_drivers::toupcam` | Toupcam/AmScope-like USB cameras | Config-backed geometry/identity plus live userspace USB backend behind `os-usb` with retained USB identity metadata and local frame source | [toupcam.md](toupcam.md) | `discover_devices`, `camera_acquisition`, `camera_stream` |
...
| `numanager_drivers::wosm` | Warwick Open-Source Microscope | Prompt-based TCP output, switch-sequence, blanking, digital-input, input-pull-up, and raw analog-input control/readback | [wosm.md](wosm.md) | `discover_devices`, `motion_stage`, `light_source`, `digital_io` |
...
| `numanager_drivers::lumenera` | Lumenera Lu130 / Bio-Rad Gel Doc EZ cameras | USB descriptor discovery for loader and imaging stages plus hidden EZ-USB firmware initialization when explicitly connected; imaging capture and exposure/gain control are not exposed because the imaging wire protocol is not recorded | [lumenera.md](lumenera.md) | `discover_devices` |
...
| `numanager_drivers::sim_lsm` | Laser-scanning microscope simulation | Confocal capture, image stream, and signal stream simulator over the shared procedural cell-culture model | [sim-lsm.md](sim-lsm.md) | `lsm_confocal_capture`, `lsm_confocal_stream`, `lsm_signal_stream`, `lsm_gui` |
