...
| `numanager_drivers::spark_cyto` | Spark Cyto | TDCL over USB for plate, detector, environment, motion, optics-carrier, injector, barcode, imaging-head, and camera workflows, with image capture served over the TDCL data channel and frame geometry read from the instrument; the reader's USB id is configured because it is not evidenced | [spark-cyto.md](spark-cyto.md) | `spark_cyto`, `environment_control`, `plate_reader` |
...
| `numanager_drivers::ez_usb` | Generic Cypress EZ-USB loaders | Passive ambiguous loader discovery for generic pre-firmware USB identities such as `04b4:8613`; driver-specific classification requires config or observed post-firmware VID/PID | [../reverse/ez-usb-renumeration.md](../reverse/ez-usb-renumeration.md) | `discover_devices` |
...
| `numanager_drivers::lumenera` | Lumenera Lu130 / Bio-Rad Gel Doc EZ cameras | USB descriptor discovery for both stages, EZ-USB firmware initialization, sensor-pipeline configuration and register load at open, and hardware-validated live single-frame `CameraCapture` at 1392x1040 `Raw16` with writable `exposure`; `gain` fails closed because its register mapping is unevidenced | [lumenera.md](lumenera.md) | `gel_doc` |
