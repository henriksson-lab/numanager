...
| ToupTek camera registry | [`toupcam-model-registry.md`](toupcam-model-registry.md) | 665 camera variants (name, USB product id, geometry, pixel pitch, resolutions) recovered from the vendor runtime by disassembling the per-model builder routines; validated against two cameras whose geometry was known independently. Open sequences and the register encoding are not recovered, so the registry gives identification and geometry only |
| Toupcam U3CMOS03100KPA | [`toupcam-u3cmos03100kpa.md`](toupcam-u3cmos03100kpa.md) | USBPcap capture of the vendor application establishes model identity, 2048x1534 RAW8 geometry, the 1-byte frame trailer, the short-transfer frame delimiter, and a 48-transfer open sequence; live open, streaming, and frame capture are hardware-validated through numanager. Exposure and gain remain unsupported because their `0x0b` register payloads are obfuscated with a session-dependent key — two captures of the same UI action use different register indices, and one exposure value produced two different payloads |
