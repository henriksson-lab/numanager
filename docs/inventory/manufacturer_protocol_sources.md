...
| 3Z Optics | `3Z_Optics` | Vendor product pages confirm IRIS light-source serial control and publish device-level specs; Modbus RTU framing is public; device register map is source-backed by audited Micro-Manager adapter code until an official register map is obtained | mixed | Upstream Micro-Manager added `3Z_Optics` in June 2026 for 3Z IRIS LED light sources using USB serial Modbus-style frames. Official pages for IRIS-400, IRIS-400HP/P, and IRIS-600HP/P confirm channel counts, wavelength ranges, TTL/serial control modes, intensity ranges, and USB serial protocol availability, but the downloadable manuals appear gated behind a form and no public register-map PDF was found. The current `numanager_drivers::three_z_optics` implementation uses the audited adapter source for semantic register/coil mappings and records that provenance; hardware validation or an official manual is still needed before claiming bench-validated support for serial settings, model ids, brightness limits, dirty-bit behavior, fault behavior, and optical output. Sources checked: <https://3zoptics.com/products/index2.html>, <https://3zoptics.com/products/index12.html>, <https://3zoptics.com/products/index3.html>, <https://3zoptics.com/service/download.html>. |
...
| Cobolt / Hübner lasers | `Cobolt`, `CoboltOfficial` | Public serial command behavior; manufacturer-authored Micro-Manager adapter exposes newer generation command families | yes, if manual obtained | The current `numanager_drivers::cobolt` support implements opt-in serial startup readback, emission/power/current/mode/autostart control, interlock/fault/usage telemetry, and query-backed refresh helpers for the classic Cobolt command surface. Current CoboltOfficial upstream has 2025-2026 changes for 05/Gen5 lasers, 12 V MLD/DPL variants, 5 V shutter-command handling, and Skyra line-addressed controls. Candidate gaps include `laser:*` and `system:input:*` commands, `autostart:abort`/`autostart:restart`, `fault:clear`, firmware/key-switch/state queries, modulation input voltage/impedance, analog/digital modulation enablement, modulation high/low current and power setpoints, and Skyra per-line power/current/wavelength/runmode commands. These can be implemented from recorded Micro-Manager source provenance; hardware validation, a manufacturer manual revision, or captured traffic is still needed before claiming generation-specific safety, warmup, CDRH delay, interlock reset, fault recovery, modulation, hardware trigger, and shutter behavior as bench-validated. Sources checked: <https://micro-manager.org/CoboltOfficial>, <https://micro-manager.org/Cobolt>, <https://github.com/micro-manager/mmCoreAndDevices/tree/main/DeviceAdapters/CoboltOfficial>. |
...
| ASI-compatible `WOSM`, `TriggerScope`, custom MCU devices | several | usually project-defined | in-tree/project docs primary | Use source/firmware where the hardware protocol was created by the adapter authors. The WOSM MCU command page for firmware code base `v0.900` is now the primary WOSM command source; the current driver uses documented `stg_out_*`, `dig_out`, `dig_in`, and `dac_dest` commands for public stage, digital, and light control, while legacy sequence, blanking, pull-up, and raw analog-input commands remain source-backed by prior adapter/reverse evidence. The current TriggerScope support implements opt-in serial direct-control, constrained TTL/DAC/focus sequence programming, and public timing-plan mapping; route mapping, camera-trigger sequence mapping, response/error vocabulary, and exact timebase semantics are not recorded. |
